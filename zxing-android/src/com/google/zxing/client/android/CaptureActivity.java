...
public final class CaptureActivity extends ActionBarActivity implements SurfaceHolder.Callback {
    ...
    public void onCreate(Bundle icicle) {
        ...
        zxingCaptureLayoutResourceId = R.layout.zxing_capture;
        zxingCaptureLayoutToolbarId = zxingCaptureLayoutThemeId = zxingCaptureLayoutMenuId = -1;


        if (extras != null) {
            if (extras.containsKey(ZXING_CAPTURE_LAYOUT_ID_KEY)) {
                zxingCaptureLayoutResourceId = extras.getInt(ZXING_CAPTURE_LAYOUT_ID_KEY, R.layout.zxing_capture);
            }
            if (extras.containsKey(ZXING_CAPTURE_LAYOUT_TOOLBAR_ID)) {
                zxingCaptureLayoutToolbarId = extras.getInt(ZXING_CAPTURE_LAYOUT_TOOLBAR_ID, -1);
            }
            if (extras.containsKey(ZXING_CAPTURE_LAYOUT_THEME_ID)) {
                zxingCaptureLayoutThemeId = extras.getInt(ZXING_CAPTURE_LAYOUT_THEME_ID, -1);
            }
        ...
        if (zxingCaptureLayoutThemeId > 0) {
            setTheme(zxingCaptureLayoutThemeId);
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.L) {
                setStatusBar(zxingCaptureLayoutThemeId);
            }
        }

        setContentView(zxingCaptureLayoutResourceId);

        if (zxingCaptureLayoutToolbarId > 0) {
            initializeToolbar(zxingCaptureLayoutToolbarId);
        }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        if(zxingCaptureLayoutMenuId>0){
            getMenuInflater().inflate(zxingCaptureLayoutMenuId, menu);
        }
        return true;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        //Return to calling activity with action code
        Intent data = new Intent();
        data.putExtra(RESULT_CODE_ITEM_MENU_ID,id);
        setResult(RESULT_OK,data);
        finish();
        return super.onOptionsItemSelected(item);
    }

    private void setStatusBar(int styleResourceId){
        // The attributes you want retrieved
        int[] attrs = {android.R.attr.statusBarColor};

        // Parse MyCustomStyle, using Context.obtainStyledAttributes()
        TypedArray ta = obtainStyledAttributes(styleResourceId, attrs);

        // Fetch the text from your style like this.
        String text = ta.getString(2);

        // Fetching the colors defined in your style
        int statusBarColor = ta.getColor(0, Color.WHITE);
